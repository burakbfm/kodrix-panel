...
export default function EditLessonClient({ lesson, programId }: EditLessonClientProps) {
    const [activeTab, setActiveTab] = useState<'student' | 'teacher'>('student');

    // Split existing attachments by category
    const existingAttachments = Array.isArray(lesson.attachments) ? lesson.attachments as FileAttachment[] : [];

    // Initial file separation
    // Initial file separation with URL path fallback
    const initialDocuments = existingAttachments.filter(a =>
        a.category === 'document' || (a.url && a.url.includes('/docs/'))
    );
    const initialSlides = existingAttachments.filter(a =>
        ((a.category === 'slide' || !a.category) && (!a.url || !a.url.includes('/docs/')))
    );

    const [documents, setDocuments] = useState<FileAttachment[]>(initialDocuments);
    const [slides, setSlides] = useState<FileAttachment[]>(initialSlides);

    const [content, setContent] = useState(lesson.content || "");
    const [teacherContent, setTeacherContent] = useState(lesson.teacher_content || ""); // Load teacher notes
    ...
    async function handleSubmit(formData: FormData) {
        ...
        // Add program ID for revalidation
        ...
        <div className="p-8 max-w-5xl mx-auto space-y-8">
            ...
            <div>
                ...
                <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-3">
                    <span className="text-kodrix-purple dark:text-amber-500">Düzenle:</span> {lesson.title}
                </h1>
            </div>
